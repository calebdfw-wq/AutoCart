            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {meal.cartItems.map(item => (
                  <span
                    key={item}
                    className="px-2 py-0.5 rounded-full bg-brand-500/8 border border-brand-500/15 text-[11px] text-brand-400/90 font-medium"
                  >
                    {item}
                  </span>
                ))}
...
// ─── Page ─────────────────────────────────────────────────────
export default function CartDetailPage({ params }: PageProps) {
  const { id } = params
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div>
            <div className="flex items-start gap-3 flex-wrap mb-2">
              ...
              {cart.visibility !== 'public' && (
                <Badge variant="orange">{cart.visibility}</Badge>
              )}
            </div>

            <h1 className="font-display font-extrabold text-2xl sm:text-3xl text-white leading-tight mb-3">
              {cart.title}
            </h1>
          ...
          {cart.macrosPerServing && (
            <MacroBar macros={cart.macrosPerServing} />
          )}
          ...
          <div className="bg-surface-800 rounded-2xl border border-white/8 overflow-hidden">
            ...
            >
              <h2 className="font-display font-bold text-base text-white">
                Grocery Items
                <span className="ml-2 text-sm font-normal text-zinc-500">({cart.items.length})</span>
              </h2>
                  ...
                  <div key={category}>
                    ...
                    {items.map((item, i) => (
                      <div
                        key={item.id}
                        className={cn(
                          'flex items-start gap-4 px-5 py-3.5 hover:bg-white/2 transition-colors',
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          {item.preferredBrand && (
                            <p className="text-xs text-zinc-500 mt-0.5">Preferred: {item.preferredBrand}</p>
                          )}
                          ...
                          {item.notes && (
                            <p className="text-xs text-zinc-600 mt-0.5 italic">{item.notes}</p>
                          )}
                        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-brand-500/10 via-surface-800 to-surface-800 border border-brand-500/20 shadow-glow-sm">
            ...
            <p className="text-xs text-zinc-500 mb-4 leading-relaxed">
              Generate a complete grocery list from this cart with one click
            </p>
            <Button
              fullWidth
              size="lg"
              ...
              <div className="p-4 rounded-2xl bg-surface-800 border border-white/8">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-xs font-semibold text-white">
                    {calories} cal <span className="text-zinc-500 font-normal">/ serving</span>
                  </p>
                </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {meal.prepTimeMinutes && (
                        <p className="text-[10px] text-zinc-600 flex items-center gap-1 mt-0.5">
                          <Clock size={8} /> {meal.prepTimeMinutes} min
                        </p>
                      )}
                    </div>
                    <div className="text-right shrink-0">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={cn(
                ...
                isLiked ? 'bg-red-500/10 border-red-500/30 text-red-400' : 'bg-surface-800 border-white/10 text-zinc-400 hover:text-red-400 hover:border-red-500/30',
              )}
            >
            ...
            <button
              ...
              className={cn(
                'flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-medium transition-all',
                isSaved ? 'bg-brand-500/10 border-brand-500/30 text-brand-400' : 'bg-surface-800 border-white/10 text-zinc-400 hover:text-brand-400 hover:border-brand-500/30',
              )}
            >
              <Bookmark size={15} className={isSaved ? 'fill-current' : ''} />
              {formatNumber(cart.savesCount + (isSaved ? 1 : 0))}
            </button>
          </div>

          <div className="flex gap-2">
            <Button variant="secondary" size="md" fullWidth icon={<Share2 size={15} />} onClick={handleShare}>
              Share
            </Button>
            <Button variant="secondary" size="md" fullWidth icon={<Copy size={15} />} onClick={handleDuplicate}>
              Duplicate
            </Button>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-surface-800 border border-white/8 text-center">
            <div className="flex items-center justify-center gap-1 text-xs text-zinc-600">
              <Eye size={12} />
              {formatNumber(cart.viewsCount)} views · Updated {formatDate(cart.updatedAt)}
            </div>
          </div>
          ...
          {isOwner && (
            <Link href={`/create?edit=${cart.id}`}>
              <Button variant="outline" size="md" fullWidth icon={<Edit size={15} />}>
                Edit Cart
              </Button>
            </Link>
          )}
      ...
      {/* Generate modal */}
      {showGenerate && (
        <GenerateModal cart={cart} onClose={() => setShowGenerate(false)} />
      )}
    </div>
  )
}
