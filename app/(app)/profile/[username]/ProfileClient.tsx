...
export default function ProfileClient({ params }: Props) {
  ...
  const userCarts = DEMO_CARTS.filter(c => c.creatorId === profileUser.id && c.visibility === 'public')
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="px-5 sm:px-7 pb-6">
          <div className="flex items-end justify-between gap-4 -mt-10 mb-4">
            ...
            <div className="flex gap-2 pb-1">
              {isOwnProfile ? (
                ...
                <>
                  <Button variant={isFollowing ? 'secondary' : 'primary'} size="sm" icon={<UserPlus size={14} />} onClick={handleFollow}>
          ...
          <div className="mb-3">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {profileUser.userType === 'creator' && <Badge variant="green" dot>Creator</Badge>}
              {profileUser.userType === 'admin' && <Badge variant="orange" dot>Admin</Badge>}
            </div>
            <p className="text-sm text-zinc-500">@{profileUser.username}</p>
          </div>

          {profileUser.bio && <p className="text-sm text-zinc-400 leading-relaxed mb-4 max-w-xl">{profileUser.bio}</p>}

          {socialLinks.length > 0 && (
            <div className="flex flex-wrap gap-3 mb-4">
              {socialLinks.map(link => (
                <a key={link.label} href={link.url!.startsWith('http') ? link.url : `https://${link.prefix}${link.url}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-white transition-colors">
                  <link.icon size={13} />{link.url}
                </a>
              ))}
            </div>
          )}
          ...
          <div className="flex gap-5 text-sm">
            <div><span className="font-display font-bold text-white">{formatNumber(profileUser.followerCount ?? 0)}</span><span className="text-zinc-500 ml-1.5">followers</span></div>
            <div><span className="font-display font-bold text-white">{formatNumber(profileUser.followingCount ?? 0)}</span><span className="text-zinc-500 ml-1.5">following</span></div>
            <div><span className="font-display font-bold text-white">{userCarts.length}</span><span className="text-zinc-500 ml-1.5">carts</span></div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            ...
            <div key={stat.label} className="p-4 rounded-2xl bg-surface-800 border border-white/8 text-center">
              <div className={`w-9 h-9 rounded-xl ${stat.color} flex items-center justify-center mx-auto mb-2`}><stat.icon size={17} /></div>
      ...
      <div>
        <h2 className="font-display font-bold text-lg text-white mb-4 flex items-center gap-2">
          <TrendingUp size={18} className="text-brand-400" />Published AutoCarts ({userCarts.length})
        </h2>
        {userCarts.length > 0 ? (
          ...
          <div className="py-12 text-center rounded-2xl border border-dashed border-white/10">
            ...
            {isOwnProfile && <Link href="/create" className="inline-block mt-4"><Button size="sm">Create My First Cart</Button></Link>}
          </div>
        )}
      </div>
    </div>
